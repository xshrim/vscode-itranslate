...
async function activate(context) {
  ...
  // 选中翻译
  context.subscriptions.push(commands.registerCommand('iTranslate.select', translateProviderSelect));
...
async function translateProviderSelect() {
  const configuration = workspace.getConfiguration('iTranslate');
  const translateProviders = [
    ['google', '谷歌（谷歌旗下在线翻译服务）'],
    ['microsoft', '微软（微软旗下在线翻译服务）'],
    ['baidu', '百度（百度旗下在线翻译服务）'],
    ['youdao', '有道（网易旗下在线翻译服务）'],
    ['tencent', '腾讯（腾讯旗下在线翻译服务）']
  ]
  let res = await window.showQuickPick(translateProviders.map(item => item[1]), {
    placeHolder: '请选择翻译服务'
  });
  let target = translateProviders.find(item => item[1] === res);
  if (target) {
    await configuration.update('provider', target[0]);
