...
async function translate(word, lang) {
  ...
  try {
    ...
    lk = 'https://fanyi.baidu.com/#${lang.from}/${lang.to}/${encodeURIComponent(word)}'
    req = await request("post", url, {
      form: {
        from: lang.from,
        to: lang.to,
    ...
    let pho = translate.dict_result ? translate.dict_result.simple_means.symbols.map(d => d.ph_am).join(' ') : "";
